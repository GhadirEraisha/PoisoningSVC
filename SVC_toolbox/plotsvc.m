...
[Ax,Ay] = meshgrid(linspace(-3,3,20),linspace(-3,3,20));
...
if isequal(model.options.method,'SEP-CG') | isequal(model.options.method,'E-SVC') 
    ...
    ppatterns2(test); 
    contour( Ax, Ay, reshape(dist,5,5),[model.r model.r],'k');    
    ...
    ppatterns2(data.X(:,model.sv_ind),'ro',20);   % SV
    ppatterns2(data.X(:,model.bsv_ind),'k.',20);  % BSV   
    ppatterns2(test); 
    %contour( Ax, Ay, reshape(dist,10,10),[model.r model.r],'k');
